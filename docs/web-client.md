...
> **Status, 2026-09-02.** The client was built and it is *not* in this repository.
> It has its own git repo at `C:\Users\97250\WebstormProjects\shadchan-web` —
> which supersedes the `web-app/` directory proposed under *Repository layout and
> hosting* below; everything else in this document still holds. The web app is
> registered in the `shadchan-gilad` Firebase project (item 1 under *What has to
> happen outside the code* is done) and its config is checked in there at
> `lib/firebase.ts`. That repo carries `npm run check:contract`, which reads
> `lib/utils/enums.dart` from this one and fails if the enum wire strings drift.
